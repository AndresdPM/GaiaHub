   ...
   for index_obs, (s_ra, s_dec, footprint_str, obsid, filter, obs_id) in obs_table.loc[:, ['s_ra', 's_dec', 's_region', 'obsid', 'filters', 'obs_id']].iterrows():
      ...
      for poly in list_coo:
         ...
         if len(tuples_list) > 4:
            ...
            if os.path.isfile(HST_path+'img/'+obs_id+'/'+obs_id+'_drz.fits'):
               fc = [0,1,0,0.2]
            else:
               fc = [1,1,1,0.2]
...
def download_HST_images(data_products_by_obs, path = './'):
   ...
   move_files(path+'mastDownload/HST/', path+'img/')
   shutil.rmtree(path+'mastDownload/')
...
def hst1pass(HST_path, obs_id, HST_image, force_fmin, force_hst1pass, remove_previous_XYmqxyrd, verbose):
   ...
   output_dir = HST_path+'img/'+obs_id+'/'
   ...
   if not os.path.isfile(XYmqxyrd_filename) or force_hst1pass:
      ...
      psf_filename = './tmp/lib/STDPSFs/%s%s/PSFSTD_%s%s_%s%s.fits'%(instrument, detector, instrument, detector, filter, sm)
      gdc_filename = './tmp/lib/STDGDCs/%s%s/STDGDC_%s%s_%s.fits'%(instrument, detector, instrument, detector, filter)
      ...
      while not applied_pert(XYmqxyrd_filename) and (pert_grid > 0):
         bashCommand = "./tmp/fortran_codes/hst1pass.e HMIN=5 FMIN=%s PMAX=999999 GDC=%s PSF=%s PERT%i=AUTO OUT=XYmqxyrd OUTDIR=%s %s"%(fmin, gdc_filename, psf_filename, pert_grid, output_dir, HST_image_filename)
...
def launch_hst1pass(flc_images, HST_obs_to_use, HST_path, force_fmin = None, force_hst1pass = True, remove_previous_XYmqxyrd = True, verbose = True, n_processes = 1):
   """
...
def xym2pm_Gaia(iteration, Gaia_HST_table_field, Gaia_HST_table_filename, HST_image_filename, lnk_filename, mat_filename, amp_filename, date_reference_second_epoch, only_use_members, rewind_stars, force_pixel_scale, force_max_separation, force_use_sat, fix_mat, force_wcs_search_radius, min_stars_alignment, verbose, previous_xym2pm, mat_plots, no_amplifier_based, min_stars_amp, use_mean):
   ...
   if not os.path.isfile(lnk_filename) or not previous_xym2pm:
      ...
      bashCommand = "./tmp/fortran_codes/xym2pm_Gaia.e %s %s RACEN=%f DECEN=%f XCEN=5000.0 YCEN=5000.0 PSCL=%s SIZE=10000 DISP=%.1f NMIN=%i TIMEREF=%s%s%s%s%s%s"%(Gaia_HST_table_filename, HST_image_filename, ra_cent, dec_cent, pixel_scale, max_separation, min_stars_alignment, timeref, time, use_sat, use_mat, use_brute, use_amp)
...
def launch_xym2pm_Gaia(Gaia_HST_table, data_products_by_obs, HST_obs_to_use, HST_path, date_reference_second_epoch, only_use_members = False, preselect_cmd = False, rewind_stars = True, force_pixel_scale = None, force_max_separation = None, force_use_sat = True, fix_mat = True, no_amplifier_based = False, min_stars_amp = 25, force_wcs_search_radius = None, n_components = 1, clipping_prob = 6, min_stars_alignment = 100, use_mean = 'wmean', no_plots = False, verbose = True, previous_xym2pm = False, remove_previous_files = True, n_processes = 1, plot_name = ''):
   ...
   while not convergence:
      ...
      for index_image, (obs_id, HST_image) in data_products_by_obs.loc[data_products_by_obs['parent_obsid'].isin([HST_obs_to_use] if not isinstance(HST_obs_to_use, list) else HST_obs_to_use), ['obs_id', 'productFilename']].iterrows():
         ...
         HST_image_filename = HST_path+'img/'+obs_id+'/'+HST_image
      ...
      if only_use_members:
         ...
         pm_clustering = pm_cleaning_GMM_recursive(Gaia_HST_table.copy(), ['relative_hst_gaia_pmra_%s'%use_mean, 'relative_hst_gaia_pmdec_%s'%use_mean], data_0 = [0, 0], n_components = n_components, covariance_type = 'spherical', clipping_prob = clipping_prob, no_plots = no_plots, plot_name = '%s_PM_sel_%i'%(plot_name, iteration))
...
def plot_results(table, lnks, hst_image_list, HST_path, avg_pm, use_mean = 'wmean', use_members = True, plot_name_1 = 'output1', plot_name_2 = 'output2', plot_name_3 = 'output3', plot_name_4 = 'output4', plot_name_5 = 'output5', plot_name_6 = 'output6', plot_name_7 = 'output7', ext = '.pdf'):
   ...
   if len(hst_filters) >= 2:
      ...
      color = (table['gmag']-table[hst_filters[0]]).rename('G - %s'%hst_filters[0])
   ...
   if table.use_for_alignment.sum() < len(table):
      try:
         p1 = ax.scatter(table['ra'][table.use_for_alignment == False & id_gaia], table['dec'][table.use_for_alignment == False & id_gaia], transform=ax.get_transform('world'), s=20, linewidth = 1, facecolor='none', label='Gaia not used in alignment', edgecolor='k', alpha = 0.35, zorder = 1)
         p1 = ax.scatter(table['ra'][table.use_for_alignment == False & id_hst_gaia], table['dec'][table.use_for_alignment == False & id_hst_gaia], transform=ax.get_transform('world'), s=50, linewidth = 1, facecolor='none', label='HST+Gaia not used in alignment', edgecolor='r', alpha = 0.35, zorder = 2)
      except:
         pass
      try:
         with warnings.catch_warnings():
   ...
   try:
      p2 = ax.scatter(table['ra'][table.use_for_alignment == True & id_gaia],table['dec'][table.use_for_alignment == True & id_gaia], transform=ax.get_transform('world'), s=20, linewidth = 1, facecolor='none', label='Gaia used in alignment', edgecolor='k', zorder = 1)
      p2 = ax.scatter(table['ra'][table.use_for_alignment == True & id_hst_gaia],table['dec'][table.use_for_alignment == True & id_hst_gaia], transform=ax.get_transform('world'), s=50, linewidth = 1, facecolor='none', label='HST+Gaia used in alignment', edgecolor='r', zorder = 2)
   except:
      pass
   try:
      with warnings.catch_warnings():
   ...
   fig, axs = plt.subplots(2, 2, sharex = False, sharey = False, figsize = (12, 5.5))
   ...
   axs[0,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')

   axs[0,0].plot(bin_xhst, mean_xhst_pmra_hst_gaia + avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[0,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')

   axs[0,1].plot(bin_xhst, mean_xhst_pmdec_hst_gaia + avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[1,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')

   axs[1,0].plot(bin_yhst, mean_yhst_pmra_hst_gaia + avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[1,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')

   axs[1,1].plot(bin_yhst, mean_yhst_pmdec_hst_gaia + avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   fig, axs = plt.subplots(len(hst_filters)+1, 2, sharex = False, sharey = False, figsize = (12.5, 3.5*len(hst_filters)+1))
   ...
   axs[0,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
   axs[0,0].plot(bin_mag, mean_mag_pmra_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[0,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
   axs[0,1].plot(bin_mag, mean_mag_pmdec_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   for ii, cmd_filter in enumerate(hst_filters):
      ...
      axs[ii+1,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
      axs[ii+1,0].plot(bin_mag, mean_mag_pmra_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
      ...
      axs[ii+1,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
      axs[ii+1,1].plot(bin_mag, mean_mag_pmdec_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[0,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
   axs[0,0].plot(bin_color, mean_color_pmra_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   axs[0,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
   axs[0,1].plot(bin_color, mean_color_pmdec_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
   ...
   for ii, cmd_filter in enumerate(hst_filters):
      ...
      axs[ii+1,0].axhline(y = avg_pm['hst_gaia_pmra_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
      axs[ii+1,0].plot(bin_color, mean_color_pmra_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
      ...
      axs[ii+1,1].axhline(y = avg_pm['hst_gaia_pmdec_%s_%s'%(use_mean, use_mean)], linestyle = '-', linewidth = 0.75, color = 'k')
      axs[ii+1,1].plot(bin_color, mean_color_pmdec_hst_gaia, linestyle = '-', linewidth = 0.75, color = 'r')
...
def main(argv):  
   ...
   try:
      ...
      Gaia_table, Gaia_queries = incremental_query(query, args.area, min_gmag = args.min_gmag, max_gmag = args.max_gmag, n_processes = args.n_processes,
   ...
   """
   Gaia_table, obs_table = plot_fields(Gaia_table, obs_table, args.HST_path, min_stars_alignment = args.min_stars_alignment, no_plots = args.no_plots, name = args.base_path+args.base_file_name+'_search_footprint.png')
   ...
   if len(obs_table) > 0:
      ...
      Call hst1pass
      """
      launch_hst1pass(flc_images, HST_obs_to_use, args.HST_path, force_fmin = args.fmin, remove_previous_XYmqxyrd = args.remove_previous_files, force_hst1pass = args.hst1pass, n_processes = args.n_processes)
      """
      Call xym2pm_Gaia
      """
