            ...
            if (object_table['PMRA'].mask == False):
               ...
               if not args.silent:
                  try:
                     ...
                     args.pmra = 0.
                  try:
                     args.max_pmra = float(input('Max PMRA to download from Gaia not defined, please enter PMRA in mas/yr (Press enter to ignore): '))
                  except:
                     args.max_pmra = 999.
                  try:
                     args.min_pmra = float(input('Min PMRA to download from Gaia not defined, please enter PMRA in mas/yr (Press enter to ignore): '))
                  except:
                     args.min_pmra = -999.
            ...
            if (object_table['PMDEC'].mask == False):
               ...
               if not args.silent:
                  try:
                     ...
                     args.pmdec = 0.
                  try:
                     args.max_pmdec = float(input('Max PMDEC to download from Gaia not defined, please enter PMDEC in mas/yr (Press enter to ignore): '))
                  except:
                     args.max_pmdec = 999.
                  try:
                     args.min_pmdec = float(input('Min PMDEC to download from Gaia not defined, please enter PMDEC in mas/yr (Press enter to ignore): '))
                  except:
                     args.min_pmdec = -999.

         if (args.min_parallax is None) and not args.silent:
            try:
               args.min_parallax = float(input('Min parallax not defined, please enter parallax in mas/yr (Press enter to ignore): '))
            except:
               args.min_parallax = -999.

         if (args.max_parallax is None) and not args.silent:
            try:
               args.max_parallax = float(input('Max parallax not defined, please enter parallax in mas/yr (Press enter to ignore): '))
            except:
...
def main(argv):  
   ...
   print('TODO. Local Correction using k-neigbors. Figure 3 in https://iopscience.iop.org/article/10.3847/1538-4357/aaa3ec/pdf')
