...
def columns_n_conditions(source_table, search_type, astrometric_cols, photometric_cols, quality_cols, ra, dec, min_radius = 0.5, max_radius = 1.0, width = 1.0, height = 1.0, max_gmag_error = 0.5, max_rpmag_error = 0.5, max_bpmag_error = 0.5, min_parallax = -2, max_parallax = 1, max_parallax_error = 1.0, min_pmra = -6, max_pmra = 6, max_pmra_error = 1.0, min_pmdec = -6, max_pmdec = 6, max_pmdec_error = 1.0):
   ...
   if search_type == 'box':
      ...
      conditions = "CONTAINS(POINT('ICRS',"+source_table+".ra,"+source_table+".dec),CIRCLE('ICRS',%.8f,%.8f,%.8f))=1"%(ra, dec, max_radius) +" AND CONTAINS(POINT('ICRS',"+source_table+".ra,"+source_table+".dec), CIRCLE('ICRS',%.8f,%.8f,%.8f))=0"%(ra, dec, min_radius)
   else:
      conditions = "CONTAINS(POINT('ICRS',"+source_table+".ra,"+source_table+".dec),CIRCLE('ICRS',%.8f,%.8f,%.8f))=1"%(ra, dec, max_radius)
...
def xym2pm_Gaia(iteration, Gaia_HST_table_field, Gaia_HST_table_filename, HST_image_filename, lnk_filename, mat_filename, amp_filename, date_reference_second_epoch, only_use_members, rewind_stars, force_pixel_scale, force_max_separation, force_use_sat, fix_mat, force_wcs_search_radius, min_stars_alignment, verbose, force_xym2pm, plots, force_use_amp, min_stars_amp):
   ...
   if (iteration > 0) & (rewind_stars):
      align_var = ['ra', 'ra_error', 'dec', 'dec_error', 'hst_gaia_pmra_wmean', 'hst_gaia_pmra_wmean_error', 'hst_gaia_pmdec_wmean', 'hst_gaia_pmdec_wmean_error', 'gmag', 'use_for_alignment']
   else:
...
def launch_xym2pm_Gaia(Gaia_HST_table, data_products_by_obs, HST_obs_to_use, HST_path, date_reference_second_epoch, only_use_members = False, rewind_stars = True, force_pixel_scale = None, force_max_separation = None, force_use_sat = True, fix_mat = True, force_use_amp = False, min_stars_amp = 25, force_wcs_search_radius = None, n_components = 1, clipping_prob = 6, min_stars_alignment = 100, use_mean = 'wmean', plots = True, verbose = True, force_xym2pm = True, remove_previous_files = True, use_parallel = True, plot_name = ''):
   ...
   while not convergence:
      ...
      if only_use_members:
         if iteration == 0:
            ...
            min_HST_HST_stars = 0.85*len(Gaia_HST_table.loc[:, ['relative_hst_gaia_pmra_%s'%use_mean, 'relative_hst_gaia_pmdec_%s'%use_mean]].dropna())

            if (len(hst_filters) >= 2):
               if (len((Gaia_HST_table[hst_filters[0]] - Gaia_HST_table[hst_filters[1]]).dropna()) >=  min_HST_HST_stars ):
      ...
      # Useful statistics:
      pmra_evo.append(Gaia_HST_table.loc[np.isfinite(Gaia_HST_table['hst_gaia_pmra_%s'%use_mean]), 'hst_gaia_pmra_%s'%use_mean])
      pmdec_evo.append(Gaia_HST_table.loc[np.isfinite(Gaia_HST_table['hst_gaia_pmdec_%s'%use_mean]), 'hst_gaia_pmdec_%s'%use_mean])

      hst_gaia_pmra_lsqt_evo.append(np.nanstd( (Gaia_HST_table.loc[:, 'hst_gaia_pmra_%s'%use_mean]  - Gaia_HST_table.loc[:, 'pmra'])))
      hst_gaia_pmdec_lsqt_evo.append(np.nanstd( (Gaia_HST_table.loc[:, 'hst_gaia_pmdec_%s'%use_mean] - Gaia_HST_table.loc[:, 'pmdec'])))
      ...
      if iteration > 19:
         ...
         threshold = np.nanmean(Gaia_HST_table.loc[:, ['relative_hst_gaia_pmra_mean_error', 'relative_hst_gaia_pmdec_mean_error']].mean())/100.
   ...
   #np.savetxt('%s_pmra_evo.dat'%plot_name, pmra_evo)
   #np.savetxt('%s_pmdec_evo.dat'%plot_name, pmdec_evo)

   #np.savetxt('%s_hst_gaia_pm_lsqt_evo.dat'%plot_name, hst_gaia_pm_lsqt_evo)
   #np.savetxt('%s_pm_diff_evo.dat'%plot_name, pm_diff_evo)
      ...
      try:
         ...
         if args.max_search_radius is None:
            if (object_table['GALDIM_MAJAXIS'].mask == False):
               args.max_search_radius = max(1.0 * np.round(float(2. * object_table['GALDIM_MAJAXIS'] / 60.), 2), 0.1)
            else:
               try:
...
def main(argv):  
   ...
   # Membership selection
   parser.add_argument('--use_members', type=str2bool, default=False, help='Whether to use only member stars for the epochs alignment or to use all available stars.')
   ...
   parser.add_argument('--force_gaia_selection', type = str2bool, default = False, help='If True, the code will reject stars based on the cuts defined for Gaia astrometry and photometry wich will remove all stars without full Gaia astrometric or photometric solution. Default is False.')
   ...
   parser.add_argument('--force_fmin', type=int, default= None, help='Minimum flux above the sky to substract a source in the HST image. Default is automatic from HST integration time.')
   parser.add_argument('--force_pixel_scale', type=float, default= None, help='Pixel scale in arcsec/pixel used to compute the tangential plane during the match between epochs. Default is automatic from HST images.')
   ...
   parser.add_argument('--rewind_stars', type=str2bool, default=False, help='Force the program to use the PMs to rewind the stars to their second epoch to the first one before the matching.')
   parser.add_argument('--fix_mat', type=str2bool, default=False, help='Force to select the best stars in the MAT files to perform the alignment in the next iteration.')
   ...
   parser.add_argument('--force_wcs_search_radius', type=float, default= 2, help='When set to a radius (in arcsec), the program search the closest Gaia star to each bright star in the HST image within that distance to perform a pre-alignment between the two frames. Useful when not many stars are available. Default is None.')
   parser.add_argument('--min_stars_alignment', type=int, default = 5, help='Minimum number of stars per HST image to be used for the epochs alignment. Default 25.')
   ...
   parser.add_argument('--min_stars_amp', type=int, default = 5, help='Minimum number of stars per HST amplifier to compute transformations. Default is 25.')
