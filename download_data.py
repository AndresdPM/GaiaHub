...
def plot_fields(Gaia_stars, obs_table, name = 'test.png'):
   ...
   filter_range = [float(s.replace('F', '').replace('W', '')) for s in obs_table.filters]
   ...
   for index_obs, (footprint_str, obsid, filter) in obs_table.loc[:, ['s_region', 'obsid', 'filters']].iterrows():
      ...
      for poly in list_coo:
         ...
         ecs.append(coolwarm(filter.replace(r'F', '').replace(r'W', ''), 1))
         ...
         gaia_stars_per_obs.append(star_counts)
         ...
         ax.annotate(obsid, xy=(annotation_coo[0], annotation_coo[1]), xycoords='data', color = coolwarm(filter.replace(r'F', '').replace(r'W', ''), 1))
...
def get_fmin(i_exptime):
   ...
   return int(lower_fmin + (upper_fmin - lower_fmin) * (i_exptime - lower_exptime) / (upper_exptime - lower_exptime))
...
def xym2pm_Gaia(Gaia_HST_table, data_products_by_obs, HST_obs_to_use, HST_path, date_reference_second_epoch, only_use_members = False, force_pixel_scale = None, max_separation = 5.0, force_use_sat = True, n_components = 1, clipping_prob = 6, min_stars_alignment = 100, plots = True, verbose = True, remove_previous_files = True, plot_name = ''):
   """
   ...
   while not convergence:
      ...
      for index_image, (obs_id, HST_image) in data_products_by_obs.loc[data_products_by_obs['parent_obsid'].isin([HST_obs_to_use] if not isinstance(HST_obs_to_use, list) else HST_obs_to_use), ['obs_id', 'productFilename']].iterrows():
         ...
         if iteration == 0:
            ...
            Gaia_HST_table.update(Gaia_HST_table_field.use_for_alignment)
         ...
         try:
            ...
            lnk = pd.read_csv(f, names=header, sep = '\s+', comment='#', na_values = 0).set_index(Gaia_HST_table_field.index)
            f.close()
            ...
            print('  No match found for %s.'%(HST_image))
      ...
      try:
         Gaia_HST_table_field.drop(columns = lnks_averaged.columns, inplace = True)
      except:
         pass

      Gaia_HST_table_field = Gaia_HST_table_field.join(lnks_averaged)
      ...
      if only_use_members:
         if iteration == 0:
            ...
            if len(hst_filters) == 2:
               Gaia_HST_table_field['clustering_data'] =  manual_select_from_cmd((Gaia_HST_table_field[hst_filters[0]]-Gaia_HST_table_field[hst_filters[1]]).rename('%s - %s'%(hst_filters[0], hst_filters[1])), Gaia_HST_table_field[hst_filters[1]])
            else:
               for cmd_filter in hst_filters:
                  Gaia_HST_table_field['%s_clustering_data_cmd'%cmd_filter] =  manual_select_from_cmd((Gaia_HST_table_field['gmag']-Gaia_HST_table_field[cmd_filter]).rename('Gmag - %s'%cmd_filter), Gaia_HST_table_field['gmag'].rename('Gmag'))

               cmd_clustering_filters = [col for col in Gaia_HST_table_field.columns if '_clustering_data_cmd' in col]
               Gaia_HST_table_field['clustering_data'] = (Gaia_HST_table_field.loc[:, cmd_clustering_filters] == True).any(axis = 1)
               Gaia_HST_table_field.drop(columns = cmd_clustering_filters, inplace = True)
         else:
            Gaia_HST_table_field['clustering_data'] = True
         
         # Select stars in the PM space
         pm_clustering = pm_cleaning_GMM_recursive(Gaia_HST_table_field.copy(), ['relative_hst_gaia_pmra', 'relative_hst_gaia_pmdec'], data_0 = [0, 0], n_components = n_components, clipping_prob = clipping_prob, plots = plots, plot_name = '%s_%i'%(plot_name, iteration))
         new_use_for_alignment = pm_clustering & Gaia_HST_table_field.clustering_data

         if iteration > 4:
            print('\nWARNING: Max number of iterations reached: Something might have gone wrong!\nPlease check the results carefully.')
            convergence = True
         elif iteration > 0:
            convergence = np.array_equal(Gaia_HST_table_field['use_for_alignment'].values, new_use_for_alignment.values)

         if not convergence:
            Gaia_HST_table_field['use_for_alignment'] = new_use_for_alignment
   ...
   if plots:
      ...
      ax1.plot(Gaia_HST_table_field.pmra, Gaia_HST_table_field.pmdec, '.', ms = 1, label = 'Gaia')
      ax1.plot(Gaia_HST_table_field.relative_hst_gaia_pmra, Gaia_HST_table_field.relative_hst_gaia_pmdec, '.', ms = 1, label = 'HST+Gaia')
      ax1.set_xlabel('PMRA [mas/yr]')
      ax1.set_ylabel('PMDEC [mas/yr]')
      ax1.grid()

      ax2.plot(Gaia_HST_table_field.gmag, np.sqrt(Gaia_HST_table_field.pmra_error**2+Gaia_HST_table_field.pmdec_error**2), '.', ms = 1, label = 'Gaia')
      ax2.plot(Gaia_HST_table_field.gmag, np.sqrt(Gaia_HST_table_field.relative_hst_gaia_pmra_error**2+Gaia_HST_table_field.relative_hst_gaia_pmdec_error**2), '.', ms = 1, label = 'HST+Gaia error')
      ax2.plot(Gaia_HST_table_field.gmag, np.sqrt(Gaia_HST_table_field.relative_hst_gaia_pmra_std**2+Gaia_HST_table_field.relative_hst_gaia_pmdec_std**2), '.', ms = 1, label = 'HST+Gaia std')
      ax2.set_ylim(0, np.sqrt(Gaia_HST_table_field.pmra_error**2+Gaia_HST_table_field.pmdec_error**2).max())
      ax2.set_xlabel('Gmag')
   ...
   return Gaia_HST_table_field#.drop(columns = ['clustering_data'])


def median_error(x_i):
   """
   Return the median and the its error.
   """

   var_err = (1.25404*np.std(x_i)/np.sqrt(len(x_i))) #The formula is 1.25*sigma/sqrt(N) for medians of normal distributions. This is the upper limmit error and the real error should be lower than this.
   var_median = np.median(x_i)

   return var_median, var_err
...
def get_errors(data, used_cols = None):
   """
   Obtain corresponding errors for the cols.
   """
   ...
   for col in used_cols:
      if '%s_error'%col in cols:
         ...
         errors['%s_error'%col] = np.zeros_like(data[col])
...
def absolute_pm(table, use = 'all'):
   """
   This routine computes the absolute PM.
   """
   ...
   table['hst_gaia_pmra'] = table.relative_hst_gaia_pmra + pm_differences_weighted.pmra
   table['hst_gaia_pmdec'] = table.relative_hst_gaia_pmdec + pm_differences_weighted.pmdec
   
   table['hst_gaia_pmra_error'] = np.sqrt(table.relative_hst_gaia_pmra_error**2 + pm_differences_weighted.pmra_error**2)
   table['hst_gaia_pmdec_error'] = np.sqrt(table.relative_hst_gaia_pmdec_error**2 + pm_differences_weighted.pmdec_error**2)
...
def plot_results(table, plot_name = 'output.pdf'):
   """
   Plot results
   """
   ...
   ax1.plot(table.pmra, table.pmdec, 'k.', ms = 1)
   ax1.grid()
   ax1.set_xlabel(r'$\mu_{\alpha*}$ [m.a.s./yr.]')
   ax1.set_ylabel(r'$\mu_{\delta}$ [m.a.s./yr.]')
   try:
      ax1.set_xlim(table.hst_gaia_pmra.mean()-5*table.hst_gaia_pmra.std(), table.hst_gaia_pmra.mean()+5*table.hst_gaia_pmra.std())
      ax1.set_ylim(table.hst_gaia_pmdec.mean()-5*table.hst_gaia_pmdec.std(), table.hst_gaia_pmdec.mean()+5*table.hst_gaia_pmdec.std())      
   except:
      pass
   add_inner_title(ax1, 'Gaia' , loc=1)


   ax2.plot(table.hst_gaia_pmra, table.hst_gaia_pmdec, 'r.', ms =1)
   ax2.grid()
   ax2.set_xlabel(r'$\mu_{\alpha*}$ [m.a.s./yr.]')
   ax2.set_ylabel(r'$\mu_{\delta}$ [m.a.s./yr.]')
   try:
      ax2.set_xlim(table.hst_gaia_pmra.mean()-5*table.hst_gaia_pmra.std(), table.hst_gaia_pmra.mean()+5*table.hst_gaia_pmra.std())
      ax2.set_ylim(table.hst_gaia_pmdec.mean()-5*table.hst_gaia_pmdec.std(), table.hst_gaia_pmdec.mean()+5*table.hst_gaia_pmdec.std())      
   except:
      pass
   add_inner_title(ax2, 'HST+Gaia' , loc=1)

   ax3.plot(table.gmag, np.sqrt(table.pmra_error**2+table.pmdec_error**2), 'k.', ms = 1, label = 'Gaia')
   ax3.plot(table.gmag, np.sqrt(table.hst_gaia_pmra_error**2+table.hst_gaia_pmdec_error**2), 'r.', ms = 1, label = 'Mean error')
   ax3.plot(table.gmag, np.sqrt(table.relative_hst_gaia_pmra_std**2+table.relative_hst_gaia_pmdec_std**2), 'r.', ms = 1, label = '$\sigma$', alpha = 0.35)
   ax3.grid()
   try:
...
def main(argv):  
   ...
   Inputs
   """
   parser = argparse.ArgumentParser(description="This script derives proper motions (PM) combining HST annd Gaia data.")
   ...
   parser.add_argument('--max_separation', type=float, default= 5.0, help='Maximum allowed separation in pixels during the match between epochs. Default 5 pixels.')
   ...
   parser.add_argument('--min_stars_alignment', type=int, default = 500, help='Minimum number of stars per HST image to be used for the epochs alignment. Default 100.')
   ...
   parser.add_argument('--clipping_prob_pm', type=float, default=6., help='Sigma used for clipping pm and parallax. Default is 3.')
   ...
   print(Table.from_pandas(data_products_by_obs.loc[:, ['parent_obsid', 'obs_id', 'description', 'i_exptime', 'filters']]))
   print('\n')
   ...
   Call xym2pm_Gaia
   """
   Gaia_table_hst = xym2pm_Gaia(Gaia_table.copy(), data_products_by_obs, HST_obs_to_use, args.HST_path, args.date_second_epoch, only_use_members = args.use_members, force_pixel_scale = args.pixel_scale, max_separation = args.max_separation, force_use_sat = args.force_use_sat, n_components = args.pm_n_components, clipping_prob = 6, min_stars_alignment = args.min_stars_alignment, plots = args.plots, verbose = args.verbose, remove_previous_files = True, plot_name = args.base_path+'PM_selection')
   ...
   """
   plot_results(Gaia_table_hst, plot_name = args.base_path+args.base_file_name+'_result.pdf')
