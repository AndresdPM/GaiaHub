...
def check_mat(mat_filename, iteration, alpha = 0.01, plots = True, verbose= True):
   """
   ...
   if verbose:
      if (p < alpha) | ((mat.mean(axis = 0) > 1e-5).all()) | (len(mat) < 6):
         print('   WARNING:')
      if (p < alpha):
         print('   Non Gaussian distribution found in the transformation')
      if ((mat.mean(axis = 0) > 1e-5).all()):
         print('   Not (0,0) average of the distribution')
      if (len(mat) < 6):
         print('   Less than 3 stars used during the transformation')

   valid = (p > alpha) & (mat.mean(axis = 0) < 1e-5).all() & (len(mat) > 6)
   ...
   while not convergence:
      ...
      for index_image, (obs_id, HST_image) in data_products_by_obs.loc[data_products_by_obs['parent_obsid'].isin([HST_obs_to_use] if not isinstance(HST_obs_to_use, list) else HST_obs_to_use), ['obs_id', 'productFilename']].iterrows():
         ...
         try:
            ...
            if check_mat(mat_filename, iteration, alpha = alpha, plots = plots, verbose = verbose):
               ...
               if verbose:
                  print('   Bad quality match. Skipping image.')
         except:
